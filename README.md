...
这 16 帧不是 16 种行走方向，而是 Codex Pets v2 用来表现桌宠“看向指针”的方向帧。Codex 根据指针相对桌宠的位置，从图集最后两行选择最接近的角度，让角色的眼睛和头部朝向目标；指针处在中间死区时则使用普通待机帧。它们让桌宠显得会观察屏幕、视线转动更连续；不会增加新的任务状态，也不需要用户手动切换。
