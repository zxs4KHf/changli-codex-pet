...
## 2026-07-29 — 候选恢复、证据重算与事务安装

- 新增只读 `candidate_resume_doctor.py`：检查候选依赖图、下一可执行任务、五参考图限制、色键、标准图集、四基准语义、恢复输入及 SHA-256，并输出可移动的 checkpoint。
- 发布校验器不再只相信既有 `ok/pass`：现在重新计算三份盲测的严格多数，锁定答案表契约和基准硬门槛，强制 16 方向固定顺序与证据字段，从发布图集重算联系表/Look 图内容，并用 `release-evidence.json` 绑定全部 QA 媒体哈希。
- 新增受版本控制的 `pet/changli/checksums.json`；安装前验证 manifest、WebP 文件头及两个发布文件的 SHA-256。
- Windows 安装改为独占锁与同卷目录交换。交换后发生任何错误都会恢复旧目录；如果恢复本身失败，旧包的 rollback 目录会被保留并明确报告，绝不会被临时清理误删。
- 尺寸归一化和左右镜像工具改为安全默认：优先写新文件或目录；原地模式必须显式声明、校验源哈希并使用不可变备份；批量归一化先完整预检并在失败时恢复全部源帧。
- CI 新增负例单元测试、安装回滚模拟、损坏源拒绝、并发锁测试，以及中文空格路径下的 Windows PowerShell 5.1 安装测试。
- 本次没有修改桌宠图集；稳定 SHA-256 仍为 `5BC49E16AEA040C5D5101ED4FC8DCB174942079ECD98B18EAE091BBD2B2220FC`。
